...
<nav class="navbar navbar-inverse">
  ...
  <li><a  class="active" href="ADDImage.php">Add Image</a></li>
  
  
     <div class="dropdown">
	  <a href='uploadi.php?hello=true'><span class="glyphicon glyphicon-bell"></span></a></span>
...
// Check connection
		...
		$sql=null;
         $sql = "INSERT INTO add_img (id,file,time,name) VALUES (".$_SESSION['id'].",'".$target_file."',NOW(),'".$_SESSION['name']."')";
         $result = $conn->query($sql);
           $i=0;
         if ($result === TRUE) {
...
if ($result->num_rows > 0) {
    // output data of each row
    while($row = $result->fetch_assoc()) {
		$sql="INSERT INTO notification(user1,user2,type,time,read1) VALUES ('".$_SESSION['name']."','".$row['name']."','image',NOW(),0)";
